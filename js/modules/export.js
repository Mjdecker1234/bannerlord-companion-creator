...
function setupExportListeners() {
    ...
    // Download Vortex Mod as ZIP
    const downloadVortexZipBtn = document.getElementById('download-vortex-zip');
    if (downloadVortexZipBtn) {
        downloadVortexZipBtn.addEventListener('click', downloadVortexModAsZip);
    }
    
    // Download Vortex Mod (individual files)
...
async function downloadVortexModAsZip() {
    const state = getState();
    const modName = (state.name || 'Companion').replace(/\s+/g, '');
    const folderName = `${modName}Companion`;
    ...
    showNotification('Creating ZIP file...', 'success');
    ...
    try {
        ...
        // Create a README with installation instructions for ZIP
...
2. **Install with Vortex**:
   ...
   - Select the extracted ${folderName} folder or the ZIP file directly
...
4. Copy the extracted ${folderName} folder into Modules
...
## 🎯 NEW: In-Game Spawning Mod Available!

Want to spawn companions with a button click? Check out the **In-Game Companion Spawner Mod**!

Features:
- ✅ Spawn companions directly in-game using MCM menu
- ✅ Smart detection - only spawns companions that don't exist yet
- ✅ Dead companions stay dead (no resurrection)
- ✅ Works with all companions created with this web tool

**Installation:**
1. Visit the GitHub repository: https://github.com/Mjdecker1234/bannerlord-companion-creator
2. Follow the instructions in INSTALLATION.md
3. Add your companion XML to the mod's ModuleData folder
4. Use MCM in-game to spawn companions!
...
`;
        ...
        zip.file(`${folderName}/SubModule.xml`, subModuleXml);
        zip.file(`${folderName}/ModuleData/spcharacters.xml`, companionsXml);
        ...
        showNotification('ZIP file downloaded successfully! Ready for Vortex installation.', 'success');
    } catch (error) {
...
export { exportJSON, downloadJSON, exportXML, downloadXML, downloadVortexModAsZip, downloadVortexMod, generateLink, importJSON };
